"""
A collection of driver classes for different single phase DC motors. Includes Solonoid (parent base class), BiMotor & PhasedMotor (children of Solonoid)
"""
import math
...
class Solonoid(object):
    def __init__(self, pins, rampTime=100):
        ...
        self.TBC = False
        # time in milliseconds to change/ramp speed from self.value to self.finSpeed
        self._dt = rampTime
    ...
    def _smooth(self):
        ...
        while timeI < (self.finSmooth - self.initSmooth) and self.TBC:
            ...
            delta_speed = (
                1 - math.cos(timeI / float(self.finSmooth - self.initSmooth) * math.pi)) / 2
            ...
            self.value = (delta_speed * (self.finSpeed -
                                         self.initSpeed) + self.initSpeed) / 100.0
    ...
    def cellerate(self, finSpeed):
        ...
        """
        self.finSpeed = max(-100,
                            min(100, round(finSpeed * 100)))  # bounds check
        # integer of milliseconds
        self.initSmooth = int(time.monotonic() * 1000)
